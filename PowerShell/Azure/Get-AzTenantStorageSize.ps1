# This script finds all storage accounts in a tenant and writes out the storage account names and sizes to a CSV
...
foreach ($subscription in $subscriptions) {
    ...
    Start-Sleep -Seconds 5 # may be helpful in avoiding getting throttled
        ...
        foreach ($storageAccount in $storageAccounts) {
            ...
            Start-Sleep -seconds 1 # This may be necessary to avoid timeouts
#        }
    }
}
...
#Export-csv -path ./AzureTenantStorageSize.csv -usequotes asneeded -NoTypeInformation
