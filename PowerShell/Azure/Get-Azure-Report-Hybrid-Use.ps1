...
Function Set-OutputLogFiles {
    ...
    if($CSVUniqueFileNames.IsPresent) {
        $script:OutputFolderFilePathCSV = "$OutputFolderPath\azure-hybrid-use-audit-report_$($FileNameDataTime).csv"
    } else {
        $script:OutputFolderFilePathCSV = "$OutputFolderPath\azure-hybrid-use-audit-report.csv"
    }
}
